    ...
    private handleCollisions(dt: number) {
        ...
        this.state.nodes.forEach((node, nodeId) => {
            ...
            if (hitters.length > 1) {
                ...
                // Disconnected players should not capture neutral/enemy nodes 
                const playerState = this.state.players.get(p as string);
                if (playerState && !playerState.connected && node.owner !== p) {
                    return; // Skip capturing if they dropped
                }
